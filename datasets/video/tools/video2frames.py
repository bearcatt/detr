    ...
    while frame < end:  # lets loop through the frames until the end
        ...
        if frame % every == 0:  # if this is a frame we want to write out based on the 'every' argument
            ...
            if not os.path.exists(save_path) or overwrite:  # if it doesn't exist or we want to overwrite anyways
                h, w, _ = image.shape
                if h < w:
                    w_delta = int(math.ceil((w - h) / 2))
                    h_delta = 0
                    size = h
                else:
                    h_delta = int(math.ceil((h - w) / 2))
                    w_delta = 0
                    size = w

                image = image[h_delta: h - h_delta, w_delta: w - w_delta, :]
...
if __name__ == '__main__':
    ...
    frame_path = os.path.join(args.root, "frames")
    for video_name in os.listdir(raw_path)[:10000][::-1]:
        video_path = os.path.join(raw_path, video_name)
        # video_to_frames(video_path=video_path, frames_dir=frame_path, overwrite=False, every=1, chunk_size=500)
        print(video_path)
        extract_frames(video_path=video_path, frames_dir=frame_path, overwrite=False, every=1)
